package com.zdjc.report.configclass.tableclass.monthreport;
 ...
 *
 */
public class SedimentationTableClassForMonth implements BastTableClass {	
	...
	public static void customizationTableClass(XWPFTable tableOne,Object obj){
				...
				for(int j=1;j<=5;j++){//定位列
					...
					//一个ReportWeekData_Head，对应表格的一列数据,这里循环的方式以一列一列的方式填充，将已经填充过的数据直接移除集合,始终取model里面的第一个没被填充的数据
					...
					//进行数据填充									
					for(int i=3;i<=29;i++){//定位行 第6到30行是动态填充的，4到7行填入约定的表头类容
						...
						if(i>=3 && i<=6){//设置每列数据的表格头
							...
							if(i==3)Wordl2007Utis.setCellText(rowTemp,"传感器编号:"+reportWeekData_Head.getSensorNumber(),"000000",1500,"微软雅黑","000000",8,true);
							if(i==4)Wordl2007Utis.setCellText(rowTemp,"终端号:"+reportWeekData_Head.getSmuNumber(),"000000",1500,"微软雅黑","000000",8,true);
							if(i==5)Wordl2007Utis.setCellText(rowTemp,"采集器通道:"+reportWeekData_Head.getSmuChannel(),"000000",1500,"微软雅黑","000000",8,true);
							if(i==6)Wordl2007Utis.setCellText(rowTemp,"测点:"+reportWeekData_Head.getMonitorPoint(),"000000",1500,"微软雅黑","000000",8,true);
						}else{//设置每列数据的数据值
							
							ReportWeekData_Dates reportWeekData_Date = null;
							
							if(!reportWeekData_Dates.isEmpty()){
								//将ReportWeekData_Head中的承载数据的实体类取出,每一个reportWeekData_Date是一个单元格的数据
								//将数据取出填充完成之后,填充过的数据会remove掉，reportWeekData_Dates.get(0)始终是取得最新数据
								reportWeekData_Date = reportWeekData_Dates.get(0);
																
								//定位第j列第i行的单元格，这个位置用来填充监测到的具体数据
								XWPFTableCell rowTemp = tableOne.getRow(i).getCell(j);
								
								//这个单元格始终定位在第一列,用来填充数据采集到的日期
								...
								//不会打乱顺序出现日期和数据对应不上的情况。 会将缺少的数据用"暂无数据"替代,
	...
	public CreateTableConfig getTableClassConfig(XWPFDocument doc2, XmlCursor cursor,Object obj) {
		...
		return new CreateTableConfig( doc2,  cursor,  38, 6, obj);
	}
	/**
	 * 创建表格的标题 ,实用边坡表格模板
	 * @param doc2
	...
	private static void fillData(List<ReportWeekData_Head> reportWeekData_Heads){
		...
		for(int i=0;i<reportWeekData_Heads.size();i++){
			...
			if(count==dayOfweek){//如果取到数据条数等于月天数,则把该条数据当做参考
			...
			for(int f=0;f<reportWeekData_Heads.size();f++){
				...
				if(!findLostData(baseTime,c)){
					...
					if(i==basePoint.size()-1){//如果是缺少最后一条数据
						...
						reportWeekData_Dates.setTotalLaserChange("暂无数据");
